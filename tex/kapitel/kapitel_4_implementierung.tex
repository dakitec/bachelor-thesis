...
Dieses Kapitel geht auf die Einzelheiten und die Abläufe der Portierung des Laufplaners nach \ac{ROS} und Gazebo ein. Zunächst werden die bestehenden Laufplaner analysiert. Dann wird ein eigenes \ac{ROS}-Paket entworfen, welches die Basis für den neuen Laufplaner darstellt. Das Kapitel stellt alle nötigen Schritte dar, um die Simulation aufzusetzen und den Roboter dazu zu bringen, Fußbewegungen auszuführen.
...
Die Generierung erfolgt durch die Iteration über die von einem Algorithmus generierten Schritte für die Fußpositionen. Statt für die Generierung eine xml-Bibliothek zu nutzen, reicht hier eine einfache Ausgabe in einer Datei mittels \textit{ofstream}.
...
Code-technisch ist das mit drei Variablen umgesetzt, die die aktuelle Zwischenposition bestimmen. Die erste Variable gibt die maximale Zahl der Zwischenpositionen an (\textit{maxTicks}). Die zweite Variable gibt die aktuelle Stelle (\textit{tick}) an, so dass der Algorithmus über die Funktion ausrechnen kann, was die nächste Position sein wird. Die Variable \textit{diff} ist der Vektor von der Start- zur Zielposition. Damit ergibt sich für die Berechnung einer Zwischenposition die Formeln in \autoref{kap4:cosinterpolationcode}, \autoref{kap4:cosinterpolationcode2} und \autoref{kap4:cosinterpolationcode3} :
\begin{eqnarray}
\label{kap4:cosinterpolationcode}
x = diff.x() * (-0.5 * cos(\pi * tick / maxTicks) + 0.5) \\
\label{kap4:cosinterpolationcode2}
y = diff.y() * (-0.5 * cos(\pi * tick / maxTicks) + 0.5) \\
z = diff.z() * (-0.5 * cos(\pi * tick / maxTicks) + 0.5)
\label{kap4:cosinterpolationcode3}
\end{eqnarray}

In jedem Schritt wird der \textit{tick} um eins nach oben gesetzt, bis er bei der Grenze von \textit{maxTicks} angekommen ist. Dann wird dieser zurückgesetzt und die nächste Bewegung wird ausgeführt.
...
todo wie funktioniert dreifußgang - vllt mit quelle?
wie ist es aufgesetzt
